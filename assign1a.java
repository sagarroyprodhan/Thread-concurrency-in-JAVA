...
class assign1a {
	...
	public static class hashed implements Comparable<hashed>{
		...
		public int arr[];
		//public int arrnew[];
		//public Map<String,vector<int>> map;
	        ...
	        String [] tokens = line.split("\\s+");
	        	...
	        	while(k<n)
	        	{
	        		...
	        		if(str[i].equals(tokens[k]))
	        		{
	        			//o.arr[i]=k+lineno;
	        			//v[i]=new Vector();
	        			//o.v.get(i).addElement(new Integer(k+lineno));
    ...
    //System.out.println("sagar s here");
    	/*for(int i=0;i<str.length;i++){
    		for(int j=0;j<o.v.get(i).size();j++){
    			System.out.println(o.v.get(i).get(j) + " ");
    		}
    		System.out.println();
    	}*/
	...
	public static void main(String args[]){
		...
		for(int i=0;i<N;i++){
			 r[i] = obj.new MyThread(filename[i],str,o[i]);	
		}
